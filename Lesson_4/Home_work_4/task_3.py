...
Разбейте её на отдельные операции —
функции. Дополнительно сохраняйте все
...
class Atm:
    ...
    def put_money(self, value):
        ...
        Atm.LST_OPERATION.append(f"На счёт положена сумма: {value}")
        self.count += 1
        self.take_perc()
    ...
    def take_money(self, value):
        ...
        Atm.LST_OPERATION.append(f"Со счёта снята сумма: {value}")
        self.count += 1
        self.take_perc()
    ...
    def show_last_operation():
        ind = 1
        for operation in Atm.LST_OPERATION[-10:]:
            print(f"\t{ind}. {operation}")
            ind += 1
...
if __name__ == '__main__':
    ...
    print("Список 10 последних операций:")
    Atm.show_last_operation()